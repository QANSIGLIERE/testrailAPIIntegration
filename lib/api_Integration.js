...
class TestRail_API {
    ...
    async add_dataset(project_id, json_body) {
        return await post(this.url, `index.php?/api/v2/add_dataset/${project_id}`, json_body, this.headers, false);
    }

    async update_dataset(dataset_id, json_body) {
        return await post(this.url, `index.php?/api/v2/update_dataset/${dataset_id}`, json_body, this.headers, false);
    }
